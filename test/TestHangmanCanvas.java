...
public class TestHangmanCanvas {
	...
	public void testNoteIncorrectGuess() {
		...
		String incorrectGuesses = "INCORRECT_GUESS";
		int numElementsAfter = canvas.getElementCount();
		if(incorrectGuesses.length() < 3 || incorrectGuesses.length() > 6){
			assertEquals("Guessing incorrectly should add 2 element to the canvas", numElementsBefore + 2, numElementsAfter);
		}else{
			assertEquals("Guessing incorrectly should add 3 element to the canvas", numElementsBefore + 3, numElementsAfter);
		}
	}
}
